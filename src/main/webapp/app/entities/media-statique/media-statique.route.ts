...
export class MediaStatiqueResolve implements Resolve<IMediaStatique> {
  constructor(private service: MediaStatiqueService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IMediaStatique> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((mediaStatique: HttpResponse<MediaStatique>) => {
          if (mediaStatique.body) {
            return of(mediaStatique.body);
          } else {
