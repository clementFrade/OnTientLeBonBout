...
public class ReponseResource {
    ...
    public ReponseResource(ReponseRepository reponseRepository, ReponseSearchRepository reponseSearchRepository) {
    ...
    public ResponseEntity<Reponse> createReponse(@RequestBody Reponse reponse) throws URISyntaxException {
        ...
        reponseSearchRepository.save(result);
    ...
    /**
     * {@code SEARCH  /_search/reponses?query=:query} : search for the reponse corresponding
     * to the query.
     *
    ...
    public List<Reponse> searchReponses(@RequestParam String query) {
        log.debug("REST request to search Reponses for query {}", query);
        return StreamSupport
            .stream(reponseSearchRepository.search(queryStringQuery(query)).spliterator(), false)
