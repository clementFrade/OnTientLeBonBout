...
public class MusiqueResource {
    ...
    private final MusiqueSearchRepository musiqueSearchRepository;

    public MusiqueResource(MusiqueRepository musiqueRepository, MusiqueSearchRepository musiqueSearchRepository) {
        this.musiqueRepository = musiqueRepository;
        this.musiqueSearchRepository = musiqueSearchRepository;
    }
    ...
    public ResponseEntity<Musique> createMusique(@RequestBody Musique musique) throws URISyntaxException {
        ...
        musiqueSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteMusique(@PathVariable Long id) {
        ...
        musiqueSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/musiques?query=:query} : search for the musique corresponding
     * to the query.
     *
     * @param query the query of the musique search.
     * @return the result of the search.
     */
    @GetMapping("/_search/musiques")
    public List<Musique> searchMusiques(@RequestParam String query) {
        log.debug("REST request to search Musiques for query {}", query);
        return StreamSupport
            .stream(musiqueSearchRepository.search(queryStringQuery(query)).spliterator(), false)
