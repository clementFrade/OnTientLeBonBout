...
public class MediaStatiqueResource {
    ...
    private final MediaStatiqueSearchRepository mediaStatiqueSearchRepository;

    public MediaStatiqueResource(MediaStatiqueRepository mediaStatiqueRepository, MediaStatiqueSearchRepository mediaStatiqueSearchRepository) {
        this.mediaStatiqueRepository = mediaStatiqueRepository;
        this.mediaStatiqueSearchRepository = mediaStatiqueSearchRepository;
    }
    ...
    public ResponseEntity<MediaStatique> createMediaStatique(@RequestBody MediaStatique mediaStatique) throws URISyntaxException {
        ...
        mediaStatiqueSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteMediaStatique(@PathVariable Long id) {
        ...
        mediaStatiqueSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/media-statiques?query=:query} : search for the mediaStatique corresponding
     * to the query.
     *
     * @param query the query of the mediaStatique search.
     * @return the result of the search.
     */
    @GetMapping("/_search/media-statiques")
    public List<MediaStatique> searchMediaStatiques(@RequestParam String query) {
        log.debug("REST request to search MediaStatiques for query {}", query);
        return StreamSupport
            .stream(mediaStatiqueSearchRepository.search(queryStringQuery(query)).spliterator(), false)
