...
public class InviteResource {
    ...
    private final InviteSearchRepository inviteSearchRepository;

    public InviteResource(InviteRepository inviteRepository, InviteSearchRepository inviteSearchRepository) {
        this.inviteRepository = inviteRepository;
        this.inviteSearchRepository = inviteSearchRepository;
    }
    ...
    public ResponseEntity<Invite> createInvite(@RequestBody Invite invite) throws URISyntaxException {
        ...
        inviteSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteInvite(@PathVariable Long id) {
        ...
        inviteSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/invites?query=:query} : search for the invite corresponding
     * to the query.
     *
     * @param query the query of the invite search.
     * @return the result of the search.
     */
    @GetMapping("/_search/invites")
    public List<Invite> searchInvites(@RequestParam String query) {
        log.debug("REST request to search Invites for query {}", query);
        return StreamSupport
            .stream(inviteSearchRepository.search(queryStringQuery(query)).spliterator(), false)
