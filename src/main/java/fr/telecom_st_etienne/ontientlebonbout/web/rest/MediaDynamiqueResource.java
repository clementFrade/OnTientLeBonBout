...
public class MediaDynamiqueResource {
    ...
    private final MediaDynamiqueSearchRepository mediaDynamiqueSearchRepository;

    public MediaDynamiqueResource(MediaDynamiqueRepository mediaDynamiqueRepository, MediaDynamiqueSearchRepository mediaDynamiqueSearchRepository) {
        this.mediaDynamiqueRepository = mediaDynamiqueRepository;
        this.mediaDynamiqueSearchRepository = mediaDynamiqueSearchRepository;
    }
    ...
    public ResponseEntity<MediaDynamique> createMediaDynamique(@RequestBody MediaDynamique mediaDynamique) throws URISyntaxException {
        ...
        mediaDynamiqueSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteMediaDynamique(@PathVariable Long id) {
        ...
        mediaDynamiqueSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/media-dynamiques?query=:query} : search for the mediaDynamique corresponding
     * to the query.
     *
     * @param query the query of the mediaDynamique search.
     * @return the result of the search.
     */
    @GetMapping("/_search/media-dynamiques")
    public List<MediaDynamique> searchMediaDynamiques(@RequestParam String query) {
        log.debug("REST request to search MediaDynamiques for query {}", query);
        return StreamSupport
            .stream(mediaDynamiqueSearchRepository.search(queryStringQuery(query)).spliterator(), false)
