...
import fr.telecom_st_etienne.ontientlebonbout.repository.search.MusiqueSearchRepository;
...
public class MusiqueResourceIT {
     ...
     *
     * @see fr.telecom_st_etienne.ontientlebonbout.repository.search.MusiqueSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final MusiqueResource musiqueResource = new MusiqueResource(musiqueRepository, mockMusiqueSearchRepository);
    ...
    public void createMusique() throws Exception {
        ...
        // Validate the Musique in Elasticsearch
        verify(mockMusiqueSearchRepository, times(1)).save(testMusique);
    }
    ...
    public void createMusiqueWithExistingId() throws Exception {
        ...
        // Validate the Musique in Elasticsearch
        verify(mockMusiqueSearchRepository, times(0)).save(musique);
    }
    ...
    public void getAllMusiques() throws Exception {
        ...
        restMusiqueMockMvc.perform(get("/api/musiques?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].artiste").value(hasItem(DEFAULT_ARTISTE)));
    }
    ...
    public void getMusique() throws Exception {
        ...
        restMusiqueMockMvc.perform(get("/api/musiques/{id}", musique.getId()))
            ...
            .andExpect(jsonPath("$.artiste").value(DEFAULT_ARTISTE));
    }
    ...
    public void deleteMusique() throws Exception {
        ...
        // Validate the Musique in Elasticsearch
        verify(mockMusiqueSearchRepository, times(1)).deleteById(musique.getId());
    }

    @Test
    @Transactional
    public void searchMusique() throws Exception {
        // Initialize the database
        musiqueRepository.saveAndFlush(musique);
        when(mockMusiqueSearchRepository.search(queryStringQuery("id:" + musique.getId())))
            .thenReturn(Collections.singletonList(musique));
        // Search the musique
        restMusiqueMockMvc.perform(get("/api/_search/musiques?query=id:" + musique.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(musique.getId().intValue())))
            .andExpect(jsonPath("$.[*].artiste").value(hasItem(DEFAULT_ARTISTE)));
