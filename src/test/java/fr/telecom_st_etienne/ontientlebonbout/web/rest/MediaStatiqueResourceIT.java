...
import fr.telecom_st_etienne.ontientlebonbout.repository.search.MediaStatiqueSearchRepository;
...
public class MediaStatiqueResourceIT {
     ...
     *
     * @see fr.telecom_st_etienne.ontientlebonbout.repository.search.MediaStatiqueSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final MediaStatiqueResource mediaStatiqueResource = new MediaStatiqueResource(mediaStatiqueRepository, mockMediaStatiqueSearchRepository);
    ...
    public void createMediaStatique() throws Exception {
        ...
        // Validate the MediaStatique in Elasticsearch
        verify(mockMediaStatiqueSearchRepository, times(1)).save(testMediaStatique);
    }
    ...
    public void createMediaStatiqueWithExistingId() throws Exception {
        ...
        // Validate the MediaStatique in Elasticsearch
        verify(mockMediaStatiqueSearchRepository, times(0)).save(mediaStatique);
    }
    ...
    public void deleteMediaStatique() throws Exception {
        ...
        // Validate the MediaStatique in Elasticsearch
        verify(mockMediaStatiqueSearchRepository, times(1)).deleteById(mediaStatique.getId());
    }

    @Test
    @Transactional
    public void searchMediaStatique() throws Exception {
        // Initialize the database
        mediaStatiqueRepository.saveAndFlush(mediaStatique);
        when(mockMediaStatiqueSearchRepository.search(queryStringQuery("id:" + mediaStatique.getId())))
            .thenReturn(Collections.singletonList(mediaStatique));
        // Search the mediaStatique
        restMediaStatiqueMockMvc.perform(get("/api/_search/media-statiques?query=id:" + mediaStatique.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(mediaStatique.getId().intValue())));
    }
