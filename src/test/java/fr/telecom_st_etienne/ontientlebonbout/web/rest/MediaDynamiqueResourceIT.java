...
import fr.telecom_st_etienne.ontientlebonbout.repository.search.MediaDynamiqueSearchRepository;
...
public class MediaDynamiqueResourceIT {
     ...
     *
     * @see fr.telecom_st_etienne.ontientlebonbout.repository.search.MediaDynamiqueSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final MediaDynamiqueResource mediaDynamiqueResource = new MediaDynamiqueResource(mediaDynamiqueRepository, mockMediaDynamiqueSearchRepository);
    ...
    public void createMediaDynamique() throws Exception {
        ...
        // Validate the MediaDynamique in Elasticsearch
        verify(mockMediaDynamiqueSearchRepository, times(1)).save(testMediaDynamique);
    }
    ...
    public void createMediaDynamiqueWithExistingId() throws Exception {
        ...
        // Validate the MediaDynamique in Elasticsearch
        verify(mockMediaDynamiqueSearchRepository, times(0)).save(mediaDynamique);
    }
    ...
    public void deleteMediaDynamique() throws Exception {
        ...
        // Validate the MediaDynamique in Elasticsearch
        verify(mockMediaDynamiqueSearchRepository, times(1)).deleteById(mediaDynamique.getId());
    }

    @Test
    @Transactional
    public void searchMediaDynamique() throws Exception {
        // Initialize the database
        mediaDynamiqueRepository.saveAndFlush(mediaDynamique);
        when(mockMediaDynamiqueSearchRepository.search(queryStringQuery("id:" + mediaDynamique.getId())))
            .thenReturn(Collections.singletonList(mediaDynamique));
        // Search the mediaDynamique
        restMediaDynamiqueMockMvc.perform(get("/api/_search/media-dynamiques?query=id:" + mediaDynamique.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(mediaDynamique.getId().intValue())))
            .andExpect(jsonPath("$.[*].dureeSeconde").value(hasItem(DEFAULT_DUREE_SECONDE)));
